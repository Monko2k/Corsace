        ...
        <div class="qualifier" v-if="!loading">
            ...
            <div class="qualifierList" v-if="qualifier">
                <div :style="user.isReferee ? {cursor: 'pointer'} : null" @click="user.isReferee ? staffReg() : null">
                    <span class="qualifierListed bold" v-if="qualifier.referee && user.id === qualifier.referee.id">-</span><span class="qualifierListed bold" v-else-if="user.isReferee">+</span> {{ $t('match.referees') }}: <span class="qualifierListed" v-if="qualifier.referee">{{ qualifier.referee.username }}</span>
                </div>
                <div :style="user.team && user.team.captain === user.id ? {cursor: 'pointer'} : null" @click="user.team && user.team.captain === user.id ? teamReg() : null">
                    <span class="qualifierListed bold" v-if="inQualifier">-</span><span class="qualifierListed bold" v-else-if="user.team && user.team.captain === user.id">+</span> {{ $t('header.teams') }}: <span class="qualifierListed" v-if="qualifier.teams"><span v-for="(team, index) in qualifier.teams" :key="index">{{ team.name }}<span v-if="index + 1 < qualifier.teams.length">, </span></span></span>
                </div>
            </div>
