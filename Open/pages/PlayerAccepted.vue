...
export default {
    ...
    methods: {
        leave: function() {
            if(confirm(this.$i18n.messages[this.$i18n.locale].teams.team.playerRemoveSelf)) {
                axios.get('/api/team/leave').then(result => this.$emit("alert")).catch(err => alert(err));
            }
        },
        kick: function() {
            if(confirm(this.$i18n.messages[this.$i18n.locale].teams.team.playerRemove.replace("%%member%%", this.member.username)))
                axios.get('/api/team/kick?target=' + this.member.id).then(result => this.$emit("update")).catch(err => alert(err));
        },
        transferCaptain: function() {
            if(confirm(this.$i18n.messages[this.$i18n.locale].teams.team.playerCaptain.replace("%%member%%", this.member.username))) {
