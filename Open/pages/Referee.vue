...
export default {
    ...
    computed: {
        bannedMaps: function() {
            if (this.selectedMatch.bestOf && this.set === this.selectedMatch.bestOf)
                return [];
            if (this.selectedMatch.sets && this.selectedMatch.sets[this.set-1])
        ...
        remainingMaps: function() {
            let poolMaps = [].concat.apply([], this.mappool.modGroups.map(modGroup => modGroup.beatmaps));
            if (this.bannedMaps)
                poolMaps = poolMaps.filter((map) => !this.bannedMaps.some((banned) => banned.mapID === map.mapID));
            if (this.selectedMatch.sets && this.selectedMatch.sets[this.set-1] && this.selectedMatch.sets[this.set-1].picks)
                poolMaps = poolMaps.filter((map) => !this.selectedMatch.sets[this.set-1].picks.some((picked) => picked.map.mapID === map.mapID));
            return poolMaps;
        }
    },
    methods: {
        toggleStage: async function(stage) {
        ...
        map: async function(set, map, mapType) {
