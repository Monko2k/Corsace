    ...
    <div>
        <div class="qualifiers" v-if="!loading">
            <div class="qualifiersSubHeaders">
                <div @click="section='mappool'" class="qualifiersSubHeader" :class="{ qualifiersSubHeaderActive: section==='mappool' }" v-if="mappool">{{ $t('header.mappool') }}</div>
                <div @click="section='qualifiers'" class="qualifiersSubHeader" :class="{ qualifiersSubHeaderActive: section==='qualifiers' }">{{ $t('header.qualifiers') }}</div>
                <div @click="section='scores'" class="qualifiersSubHeader" :class="{ qualifiersSubHeaderActive: section==='scores' }">{{ $t('header.scores') }}</div>
            </div>
            <div v-if="section==='scores'" class="qualifiersSubHeaders" style="margin-top:20px;">
                <div @click="subSection='teams'; scoringType === 'costs' ? scoringType='sum' : null" class="qualifiersSubHeader" :class="{ qualifiersSubHeaderActive: subSection==='teams' }">{{ $t('header.teams') }}</div>
                <div @click="subSection='players'; scoringType === 'seeding' || scoringType === 'average' ? scoringType='sum' : null" class="qualifiersSubHeader" :class="{ qualifiersSubHeaderActive: subSection==='players' }">{{ $t('header.players') }}</div>
            </div>
            <div v-if="section==='scores'" class="qualifiersSubHeaders" style="margin-top:20px;">
            ...
            <div class="qualifiersTop">
                ...
                <div class="qualifiersHeader">{{ $t('header.qualifiers') }}</div>
                <div class="qualifiersTeamInside" v-if="inTeam">
                    <div class="teamPP">{{ $t('teams.your') }}</div>
...
import QualifierListComponent from "./QualifierListComponent";
